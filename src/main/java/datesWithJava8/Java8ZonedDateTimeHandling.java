...
public class Java8ZonedDateTimeHandling {
	...
	private static void showDifferencesBetweenDifferentTimezones() {
		...
		long berlinOffset = TimeUnit.MILLISECONDS.toHours(berlinTz.getRawOffset());
		long londonOffset = TimeUnit.MILLISECONDS.toHours(londonTz.getRawOffset());
		long belfastOffset = TimeUnit.MILLISECONDS.toHours(belfastTz.getRawOffset());
		System.out.println("Berlin offset : "+berlinOffset);
		System.out.println("London offset : "+londonOffset);
		System.out.println("Belfast offset : "+belfastOffset);
	}
	private static void showAllTimezonesAvailable() {
		...
		for (String id : ids) {
			...
			long hourOffset = TimeUnit.MILLISECONDS.toHours(tz.getRawOffset());
			long minOffset = Math.abs(TimeUnit.MILLISECONDS.toMinutes(tz.getRawOffset()) - TimeUnit.HOURS.toMinutes(hourOffset));
			System.out.println(tz.getID() + " offset : "+hourOffset+":"+minOffset);
		}
	}
